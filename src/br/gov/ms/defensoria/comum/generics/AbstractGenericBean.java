...
public abstract class AbstractGenericBean<ENTITY extends IGenericEntity> implements Serializable {
	...
	public String save() {
		...
		try {
			...
			afterSave();
			FacesUtilRecurso.addInfoMessage("comum.global.msg.info.sucesso");
			open();
			return back();

		} catch (Exception e) {
			FacesUtilRecurso.addErrorMessage(e.getCause().getMessage());
		}
		return "";
	}
	...
	public void remove() {
		try {
			...
			FacesUtilRecurso.addErrorMessage("N�o � poss�vel remover o registro pois est� associado a outra tabela");
		}
	...
	public String prosseguir() {
		String page = FacesUtilRecurso.getViewRoot();
