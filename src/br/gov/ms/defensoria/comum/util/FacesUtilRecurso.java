...
public class FacesUtilRecurso {
	 ...
	 *            de bundle da mensagem.
	 */
	public static void addInfoMessage(String clientId, String key, Object... params) {
		String keyvalue = ResourceBundleUtil.getKeyBundle(FacesUtilRecurso.getLocale(), key, params);
		// params);
	...
	public static String getMessageByKey(String key) {
		try {
			String keyValue = ResourceBundle.getBundle("ApplicationMessagesRecurso").getString(key);
			return keyValue;
		} catch (Exception e) {
			return key;
		}
	...
	public static List<SelectItem> getSelectItems(Class<? extends Enum> type) {
		...
		for (Enum e : type.getEnumConstants()) {
			result.add(new SelectItem(e, FacesUtilRecurso.getMessageByKey(type.getSimpleName() + "." + e.name())));
		}
		return result;
	}
	...
	public static String internationalizeEnum(Class<?> clazz, Enum key) {
		return FacesUtilRecurso.getMessageByKey(clazz.getSimpleName() + "." + key);
	}
	...
	public static Enum unInternationalizeEnum(Class<? extends Enum> type, String I18nValue) {
		for (Enum e : type.getEnumConstants()) {
			if (I18nValue.equals(FacesUtilRecurso.getMessageByKey(type.getSimpleName() + "." + e.name()))) {
				return e;
			}
		}
		return null;
